...
export default function UnlockNotification() {
  ...
  useEffect(() => {
    ...
    if (lastSeen === null) {
      // First install — mark all as seen
    ...
    const unseen = data.cats.filter(
      (c) => c.unlocked && c.unlocked_at && c.unlocked_at > lastSeen,
    );
    if (unseen.length > 0) {
      setNewCat(unseen[unseen.length - 1]);
    }
  }, [data]);

  function dismiss() {
    if (newCat?.unlocked_at) {
      localStorage.setItem(LS_KEY, newCat.unlocked_at);
    }
    setNewCat(null);
  }
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/30">
      <button
        onClick={dismiss}
        className="w-full max-w-lg bg-white rounded-t-2xl p-6 text-center shadow-xl"
      >
        <span className="text-5xl block mb-3">{newCat.emoji}</span>
        <p className="text-lg font-bold text-slate-900">
          You unlocked {newCat.name}!
        </p>
        <p className="text-sm text-slate-400 mt-1">Tap to dismiss</p>
      </button>
    </div>
  );
}
