...
export default function CatCollection() {
  ...
  return (
    <div className="bg-white rounded-xl p-4 shadow-sm">
      <h2 className="font-semibold text-slate-700 mb-3">{t("profile.catCollection")}</h2>
          ...
          return (
            <button
              ...
              className={`flex flex-col items-center gap-1 min-w-16 p-2 rounded-lg border-2 transition-colors ${
                active ? "border-[var(--theme-start)] bg-slate-50"
                  : cat.unlocked ? "border-transparent hover:border-slate-200"
                  : "border-transparent opacity-40"
              }`}
            >
              <span className="text-2xl relative">
                {cat.emoji}
                {!cat.unlocked && <Lock size={12} className="absolute -bottom-1 -end-1 text-slate-400" />}
              </span>
              <span className="text-[10px] text-slate-500 capitalize">{cat.name}</span>
              {!cat.unlocked && (
                <span className="text-[9px] text-slate-400">{cat.unlock_streak} {t("profile.streakDays")}</span>
              )}
            </button>
          );
        })}
