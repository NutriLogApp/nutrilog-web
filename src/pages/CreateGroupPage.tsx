...
export default function CreateGroupPage() {
  ...
  return (
    <div className="p-4 max-w-lg mx-auto">
      ...
      <h1 className="text-xl font-bold text-slate-900 mb-4">New Competition</h1>
      ...
      <h2 className="text-sm font-medium text-slate-500 mb-2">Select friends</h2>
      <div className="space-y-2 mb-4">
        {friends?.map((f: Friend) => (
          <button
            key={f.user_id}
            onClick={() => toggle(f.user_id)}
            className={`w-full bg-white rounded-xl p-3 shadow-sm flex items-center gap-3 border-2 transition-colors ${
              selected.has(f.user_id) ? "border-[var(--theme-start)]" : "border-transparent"
            }`}
          >
      ...
      >
        {mut.isPending ? <Loader2 size={16} className="animate-spin mx-auto" /> : "Create Competition"}
      </button>
    </div>
  );
}
