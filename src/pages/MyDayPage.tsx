...
import { deleteEntry, createEntry } from "@/services/entriesService";
...
export default function MyDayPage() {
  ...
  function logDrink(drink: DrinkOut) {
    const waterAmount = Math.round(drink.volume_ml * drink.water_pct / 100);
    if (waterAmount > 0) waterMut.mutate(waterAmount);
    if (drink.calories > 0) {
      ...
      createEntry({
        ...
        items: [{
          food_name: drink.name, food_name_he: drink.name_he,
          grams: drink.volume_ml, calories: drink.calories,
          protein_g: drink.protein_g, fat_g: drink.fat_g,
          carbs_g: drink.carbs_g, confidence: "high" as const,
        }],
  ...
  return (
    <div className="px-5 pt-6 pb-4 max-w-lg mx-auto space-y-5">
      ...
      <div className="glass-card p-4 animate-fade-up stagger-1">
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2.5">
            ...
            <span className="text-sm font-semibold" style={{ color: "var(--text-primary)" }}>{t("myday.addDrink")}</span>
          </div>
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {drinks?.map((d) => (
            <button key={d.id} onClick={() => logDrink(d)}
              className="glass-card-sm px-3 py-2 text-xs font-medium flex items-center gap-1.5 transition-all active:scale-95"
              style={{ color: "var(--text-secondary)" }}>
      ...
      {/* Food entries — tap to edit, pencil + trash icons */}
      <div className="animate-fade-up stagger-2">
        <h2 className="text-base font-bold mb-3" style={{ color: "var(--text-primary)" }}>{t("myday.food")}</h2>
