  ...
  return (
    <div className="px-5 pt-6 pb-4 max-w-lg mx-auto space-y-5">
      <div className="flex items-center justify-between animate-fade-up">
        <h1 className="text-xl font-bold tracking-tight" style={{ color: "var(--text-primary)" }}>{t("friends.title")}</h1>
        <button onClick={() => navigate("/friends/add")} className="p-2 rounded-lg" style={{ color: "var(--theme-accent)" }}>
          <UserPlus size={20} />
        </button>
      </div>
      ...
      {requests && requests.length > 0 && (
        <div className="animate-fade-up">
          ...
          {requests.map((r) => (
            <div key={r.friendship_id} className="glass-card-sm p-3 mb-2 flex items-center gap-3">
              ...
              <button onClick={() => respondMut.mutate({ id: r.friendship_id, action: "accept" })} className="text-xs px-3 py-1 rounded-lg bg-emerald-500 text-white active:scale-[0.98] transition-transform">{t("friends.accept")}</button>
              <button onClick={() => respondMut.mutate({ id: r.friendship_id, action: "decline" })} className="text-xs px-3 py-1 rounded-lg active:scale-[0.98] transition-transform" style={{ backgroundColor: "var(--bg-input)", color: "var(--text-secondary)" }}>{t("friends.decline")}</button>
            </div>
          ))}
        </div>
      )}
      ...
      {groups && groups.length > 0 && (
        <div className="animate-fade-up">
          <div className="flex items-center justify-between mb-2">
            ...
            <button onClick={() => navigate("/friends/groups/new")} className="text-xs font-medium" style={{ color: "var(--theme-accent)" }}>{t("friends.new")}</button>
          </div>
          {groups.map((g) => (
            <button key={g.group_id} onClick={() => navigate(`/friends/groups/${g.group_id}`)} className="glass-card-sm w-full p-3 mb-2 flex items-center gap-3 text-start active:scale-[0.98] transition-transform">
      ...
      <div className="animate-fade-up">
        ...
        {friends?.map((f) => (
          <div key={f.user_id} className="glass-card-sm p-3 mb-2 flex items-center gap-3">
      ...
      {friends && friends.length > 0 && groups && groups.length === 0 && (
        <button onClick={() => navigate("/friends/groups/new")} className="w-full py-3 rounded-lg text-white font-medium active:scale-[0.98] transition-transform animate-fade-up" style={{ background: "linear-gradient(135deg, var(--theme-start), var(--theme-end))" }}>
