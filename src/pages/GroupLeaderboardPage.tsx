...
export default function GroupLeaderboardPage() {
  ...
  return (
    <div className="p-4 max-w-lg mx-auto">
      ...
      <h1 className="text-xl font-bold mb-1" style={{ color: "var(--text-primary)" }}>{t("groups.leaderboard")}</h1>
      {data && <p className="text-xs mb-4" style={{ color: "var(--text-muted)" }}>{t("groups.weekOf")} {data.week_start}</p>}
      ...
      <div className="space-y-2">
        {data?.standings.map((s) => (
          <div key={s.user_id} className={`rounded-xl p-4 shadow-sm flex items-center gap-3 ${s.is_current_user ? "ring-2 ring-[var(--theme-start)]" : ""}`} style={{ backgroundColor: "var(--bg-card)" }}>
